...
import { Productitems, CartProduct } from '../../models';
...
const ShoopingCartScreen = ({ searchValue }: { searchValue: string }) => {
    ...
    const fetchProducts = async () => {
      ...
      const products = await Promise.all(
        cartProducts.map(cartProduct =>
          DataStore.query(Productitems, cartProduct.productID),
  ...
  const calculateBagSavings = (cartProducts: CartProduct[]) => {
    const bagSavings = cartProducts.reduce((sum, cartProduct) => {
      // Check if product and oldPrice exist
      if (cartProduct?.product?.oldPrice) {
        return sum + cartProduct.product.oldPrice * cartProduct.quantity;
      }
      return sum;
    }, 0);
  
    return `${bagSavings.toFixed(2)}`; // Format as currency
  };
  ...
  <View style={{ width: '90%', height: 200 }}>
    ...
    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', marginHorizontal : '5%' , padding : 4 }}>
      ...
      <Text>{calculateBagSavings(cartProducts)}</Text>
    </View>
    <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between' , marginHorizontal : '5%' , padding : 4 }}>
